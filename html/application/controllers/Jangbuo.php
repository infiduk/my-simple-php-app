<?
    class Jangbuo extends CI_Controller {									// Jangbuo클래스 선언
		...
		{
			$where=array( "no"=>$no );
			return $this->db->update( "jangbuo", $row, $where );
		}
		...
		public function del()
		{
			...
			redirect("jangbuo/lists/". $text1 .$page);										// 목록화면으로 돌아가기
		}
		public function add()
		{
			...
			else																						// 입력화면의 저장버튼 클릭한 경우
			{
				...
				$this->jangbuo_m->insertrow($data13); 

				redirect("jangbuo/lists". $text1 .$page);										//   목록화면으로 이동.
			}
		}
		public function edit()
		{
			...
			else																						// 입력화면의 저장버튼 클릭한 경우
			{
				...
				$this->jangbuo_m->updaterow($data13, $no); 
