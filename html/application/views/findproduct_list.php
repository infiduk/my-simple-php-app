  ...
  function find_text()
{
    if (!form1.text1.value)       // 값이 없는 경우, 전체 자료
        form1.action="/~four13/findproduct/lists";
    else                                    // 값이 있는 경우, text1 값 전달
        form1.action="/~four13/findproduct/lists/text1/" + form1.text1.value;
    form1.submit();
}
...
<tr>
    <td><?=$no; ?></td>
    <td><?=$gubun_name ?></td>
    <td align="left" ><a href="" onclick="SendProduct(<?=$no?>,'<?=$name?>',<?=$price?>)"><?=$row->name13; ?></a></td>
